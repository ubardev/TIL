...
class StarPoint {
  ...
  bindEvents() {
    ...
    // 마우스 클릭시 별점 상태 고정
    this.starBackgroundElement.addEventListener("click", (event) => {
      this.lockStarPoint();

      const { target } = event;
      const { point } = target.dataset;
      const starPointIndex = parseInt(point, 10);
      alert(starPointIndex);
    });
    ...
    this.starBackgroundElement.addEventListener(
      "mouseout",
